<?php
  ...
  mysqli_query($conn, "truncate table tb_movielook");
  mysqli_query($conn, "INSERT INTO tb_movielook(id,title,content,createtime) SELECT id,title,content,createtime FROM tb_movie where title like '%$keyword%' or content like '%$keyword%' or createtime like '%keyword%'") or die(("数据表访问错误,请检查该表是否存在".mysqli_error()));

  $sql = mysqli_query($conn, "select * from tb_movielook") or die("数据表访问错误,请检查该表是否存在".mysqli_error());
  $row = mysqli_fetch_object($sql);  //获取结果集
  //查找出指定id修改指定数据
  $id = $_GET['id'];
  $sql_mdy = mysqli_query($conn, "select * from tb_movie where id=$id") or die("数据表访问错误,请检查该表是否存在".mysqli_error());
